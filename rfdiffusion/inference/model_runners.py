...
class Sampler:
    ...
    def initialize(self, conf: DictConfig) -> None:
        ...
        self.target_feats = iu.process_target(self.inf_conf.input_pdb, parse_hetatom=True, center=False, device=self.device)
        self.chain_idx = None
    ...
    def load_checkpoint(self) -> None:
        ...
        self.ckpt = torch.load(self.ckpt_path, map_location=self.device, weights_only=True)
    ...
    def sample_init(self, return_forward_trajectory=False):
        ...
        self.mask_seq = torch.tensor(self.contig_map.inpaint_seq, device=self.device)[None, :]
        self.mask_str = torch.tensor(self.contig_map.inpaint_str, device=self.device)[None, :]
        ...
        self.potential_manager = PotentialManager(self.potential_conf, self.ppi_conf, self.diffuser_conf, self.inf_conf, self.hotspot_0idx, self.binderlen)
        ...
        if self.diffuser_conf.partial_T:
            assert xyz_27.shape[0] == L_mapped, (
            ...
            assert contig_map.hal_idx0 == contig_map.ref_idx0, (
            ...
            xyz_mapped = torch.full((1, 1, L_mapped, 27, 3), np.nan, device=self.device)
            ...
            atom_mask_mapped = torch.full((L_mapped, 27), False, device=self.device)
        ...
        seq_t = torch.full((1, L_mapped), 21, device=self.device).squeeze()  # 21 is the mask token
        ...
        fa_stack, xyz_true = self.diffuser.diffuse_pose(xyz_mapped, torch.clone(seq_t), atom_mask_mapped.squeeze(), diffusion_mask=self.diffusion_mask.squeeze(), t_list=t_list)
        ...
        if self.potential_conf.guiding_potentials is not None:
            if any(list(filter(lambda x: "substrate_contacts" in x, self.potential_conf.guiding_potentials))):
                assert len(self.target_feats["xyz_het"]) > 0, (
    ...
    def sample_step(self, *, t, x_t, seq_init, final_step):
        ...
        """
        msa_masked, msa_full, seq_in, xt_in, idx_pdb, t1d, t2d, xyz_t, alpha_t = self._preprocess(seq_init, x_t, t)
        ...
        with torch.no_grad():
            msa_prev, pair_prev, px0, state_prev, alpha, logits, plddt = self.model(
                msa_masked, msa_full, seq_in, xt_in, idx_pdb, t1d=t1d, t2d=t2d, xyz_t=xyz_t, alpha_t=alpha_t, msa_prev=msa_prev, pair_prev=pair_prev, state_prev=state_prev, t=torch.tensor(t), return_infer=True, motif_mask=self.diffusion_mask.squeeze().to(self.device)
            )
        ...
        if t > final_step:
            ...
            x_t_1, px0 = self.denoiser.get_next_pose(xt=x_t, px0=px0, t=t, diffusion_mask=self.mask_str.squeeze(), align_motif=self.inf_conf.align_motif)
        else:
...
class ScaffoldedSampler(SelfConditioning):
    ...
    def sample_init(self):
        ...
        ############################
            ...
            self.target_feats = iu.process_target(self.inf_conf.input_pdb, device=self.device)
            self.contig_map = self.construct_contig(self.target_feats)
            self.mappings = self.contig_map.get_mappings()
        ...
        fa_stack, xyz_true = self.diffuser.diffuse_pose(xT, torch.clone(seq_T), atom_mask.squeeze(), diffusion_mask=self.diffusion_mask.squeeze(), t_list=t_list, include_motif_sidechains=self.preprocess_conf.motif_sidechain_input)
