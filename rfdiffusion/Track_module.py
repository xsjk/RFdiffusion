...
class MSAPairStr2MSA(nn.Module):
    def __init__(self, d_msa=256, d_pair=128, n_head=8, d_state=16, d_hidden=32, p_drop=0.15, use_global_attn=False):
        ...
        self.row_attn = MSARowAttentionWithBias(d_msa=d_msa, d_pair=d_pair, n_head=n_head, d_hidden=d_hidden)
        if use_global_attn:
...
class Str2Str(nn.Module):
    def __init__(self, d_msa=256, d_pair=128, d_state=16, SE3_param={"l0_in_features": 32, "l0_out_features": 16, "num_edge_features": 32}, p_drop=0.1):
        ...
        self.sc_predictor = SCPred(d_msa=d_msa, d_state=SE3_param["l0_out_features"], p_drop=p_drop)
...
class IterBlock(nn.Module):
    def __init__(self, d_msa=256, d_pair=128, n_head_msa=8, n_head_pair=4, use_global_attn=False, d_hidden=32, d_hidden_msa=None, p_drop=0.15, SE3_param={"l0_in_features": 32, "l0_out_features": 16, "num_edge_features": 32}):
        super(IterBlock, self).__init__()
        if d_hidden_msa == None:
            d_hidden_msa = d_hidden

        self.msa2msa = MSAPairStr2MSA(d_msa=d_msa, d_pair=d_pair, n_head=n_head_msa, d_state=SE3_param["l0_out_features"], use_global_attn=use_global_attn, d_hidden=d_hidden_msa, p_drop=p_drop)
        self.msa2pair = MSA2Pair(d_msa=d_msa, d_pair=d_pair, d_hidden=d_hidden // 2, p_drop=p_drop)
        # d_hidden=d_hidden, p_drop=p_drop)
        self.pair2pair = PairStr2Pair(d_pair=d_pair, n_head=n_head_pair, d_hidden=d_hidden, p_drop=p_drop)
...
class IterativeSimulator(nn.Module):
    def __init__(
        ...
        if n_extra_block > 0:
            self.extra_block = nn.ModuleList([IterBlock(d_msa=d_msa_full, d_pair=d_pair, n_head_msa=n_head_msa, n_head_pair=n_head_pair, d_hidden_msa=8, d_hidden=d_hidden, p_drop=p_drop, use_global_attn=True, SE3_param=SE3_param_full) for i in range(n_extra_block)])

        # Update with seed sequences
        if n_main_block > 0:
            self.main_block = nn.ModuleList([IterBlock(d_msa=d_msa, d_pair=d_pair, n_head_msa=n_head_msa, n_head_pair=n_head_pair, d_hidden=d_hidden, p_drop=p_drop, use_global_attn=False, SE3_param=SE3_param_full) for i in range(n_main_block)])

        self.proj_state2 = nn.Linear(SE3_param_full["l0_out_features"], SE3_param_topk["l0_out_features"])
        # Final SE(3) refinement
        if n_ref_block > 0:
            self.str_refiner = Str2Str(d_msa=d_msa, d_pair=d_pair, d_state=SE3_param_topk["l0_out_features"], SE3_param=SE3_param_topk, p_drop=p_drop)
    ...
    def forward(self, seq, msa, msa_full, pair, xyz_in, state, idx, use_checkpoint=False, motif_mask=None):
        ...
        for i_m in range(self.n_extra_block):
            ...
            msa_full, pair, R_in, T_in, state, alpha = self.extra_block[i_m](msa_full, pair, R_in, T_in, xyz, state, idx, motif_mask=motif_mask, use_checkpoint=use_checkpoint)
            R_s.append(R_in)
            T_s.append(T_in)
        ...
        for i_m in range(self.n_main_block):
            ...
            msa, pair, R_in, T_in, state, alpha = self.main_block[i_m](msa, pair, R_in, T_in, xyz, state, idx, motif_mask=motif_mask, use_checkpoint=use_checkpoint)
            R_s.append(R_in)
            T_s.append(T_in)
            alpha_s.append(alpha)
        ...
        for i_m in range(self.n_ref_block):
            ...
            T_in = T_in.detach()
            xyz = einsum("bnij,bnaj->bnai", R_in, xyz_in) + T_in.unsqueeze(-2)
            R_in, T_in, state, alpha = self.str_refiner(msa, pair, R_in, T_in, xyz, state, idx, top_k=64, motif_mask=motif_mask)
            R_s.append(R_in)
            T_s.append(T_in)
            alpha_s.append(alpha)
